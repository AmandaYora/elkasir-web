...
| `payment` | QRIS/VA gateway (Tripay/Midtrans, selectable, ONE active wallet — PLAN.md §9.1.1): create/quote-fee/verify/parse payment across channels + registry-driven webhook dispatch (§9), PLUS (Part 3, §10) the external payment API itself — `POST /external/payments/charges`, `GET /external/payments/charges/{orderRef}/status`, `GET /external/payments/channels`, all gated `ActorApp`. Owns NO business ledger — only webhook idempotency, a thin order-ref→app_id→provider_ref dispatch index, DB-backed gateway config (encrypted), and the app registry (each external app's secret stored BOTH bcrypt-hashed, for incoming auth, AND reversibly encrypted, for outbound webhook-relay signing — §10.1.6). Its presentation layer owns `POST /webhooks/payment` (dispatch now branches: `kind=internal` → in-process consumer call; `kind=external` → fire-and-forget signed HTTP relay, §10.1.10) plus the three external routes above — superadmin config/registry routes still live in `platform`, §9.1.10 | `webhook_events`, `payment_charge_apps`, `payment_gateway_config`, `payment_clients` | `paymentclient` — create charge (channel-aware), quote fee, list channels, check status, verify/parse webhook, gateway config CRUD, app registry CRUD | `auth` (mw, incl. a narrow `payment_clients` login-lookup query for `ActorApp` token issuance + a live per-request status check, §10.1.2/§10.1.4 — same precedented pattern as its `platform_users`/`stores.status` reads); consumed by selforder/subscription (charges) and platform (config+registry passthrough); called directly over HTTP by registered `kind=external` apps (§10) |
