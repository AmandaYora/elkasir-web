...
// New assembles the platform module: repo → service → handler. Consumes 6 contracts:
...
// adminuser (tenant admin-password-reset recovery flow — the superadmin's only way to restore
// access to a tenant that lost its own admin credentials, since adminuser's own reset-password
// route is self-service/store-scoped only).
func New(pool *sql.DB, q *sqlcgen.Queries, auth authcontract.Authenticator, subscriptionClient subscriptionclient.Client, salesClient salesclient.Client, withdrawalClient withdrawalclient.Client, platformUserClient platformuserclient.Client, paymentClient paymentclient.Client, adminUserClient adminuserclient.Client) *Module {
	repo := infrastructure.NewRepo(pool, q)
	svc := application.NewService(repo, pool, subscriptionClient, salesClient, withdrawalClient, platformUserClient, paymentClient, adminUserClient)
