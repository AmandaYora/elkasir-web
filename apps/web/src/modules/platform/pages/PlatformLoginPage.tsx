...
// Superadmin sign-in — near-copy of LoginPage.tsx (§2.2: login pages differ only in copy).
export default function PlatformLoginPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    navigate(ROUTE_PATHS.platformDashboard, { replace: true });
  };
    ...
    <Card className="p-7 sm:p-8">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-tight text-text">
          Masuk ke Konsol Platform
        </h1>
        <p className="mt-1.5 text-sm text-muted">
          Khusus superadmin — kelola tenant, langganan, dan pencairan dana.
        </p>
      </div>
      ...
      <form onSubmit={onSubmit} noValidate className="mt-7 space-y-4">
        <Field
          ...
          value={email}
          onChange={setEmail}
          placeholder="superadmin@elkasir.app"
        />
      ...
      <p className="mt-6 text-center text-sm text-muted">
        Bukan superadmin?{" "}
        <a href={ROUTE_PATHS.login} className="font-medium text-text hover:underline">
          Masuk ke dasbor toko
        </a>
      </p>
    </Card>
  );
